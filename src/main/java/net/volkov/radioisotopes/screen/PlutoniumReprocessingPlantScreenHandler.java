...
import net.volkov.radioisotopes.screen.slot.ModBRFuelSlot;
import net.volkov.radioisotopes.screen.slot.ModPRFuelSlot;
import net.volkov.radioisotopes.screen.slot.ModResultSlot;

public class PlutoniumReprocessingPlantScreenHandler extends ScreenHandler {
    private final Inventory inventory;
    private final PropertyDelegate propertyDelegate;

    public PlutoniumReprocessingPlantScreenHandler(int syncId, PlayerInventory playerInventory) {
        this(syncId, playerInventory, new SimpleInventory(4), new ArrayPropertyDelegate(4));
    }

    public PlutoniumReprocessingPlantScreenHandler(int syncId, PlayerInventory playerInventory,
                                                   Inventory inventory, PropertyDelegate delegate) {
        super(ModScreenHandlers.PLUTONIUM_REPROCESSING_PLANT_SCREEN_HANDLER, syncId);
        checkSize(inventory, 4);
        this.inventory = inventory;
        inventory.onOpen(playerInventory.player);
        this.propertyDelegate = delegate;

        this.addSlot(new ModPRFuelSlot(inventory, 0, 33, 53));
